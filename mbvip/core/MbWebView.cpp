...
namespace mb {
	...
	void MbWebView::createWkeWebWindowInUiThread(mbWindowType type, HWND parent, int x, int y, int width, int height)
	{
		...
		case MB_WINDOW_TYPE_NO_TITLEBAR:
			style = WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN;  // WS_EX_WINDOWEDGE  
			break;
	...
	LRESULT MbWebView::windowProcImpl(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_DROPFILES:
			OutputDebugStringA("----mb WM_DROPFILES");
			//调用了::RegisterDragDrop后，就无法收到WM_DROPFILES消息
			break;
